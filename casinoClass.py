...
class Casino:
    ...
    def menu(self, nick_name):
        a = input("")
        player_data = self.check_nickname(nick_name)
        ...
        print("\n\n")
        if action == "1":
            self.Magic(player_data_for_magic, nick_name, my_points)
        elif action == "2":
            self.Blackjack(player_data_for_Blackjack, nick_name, my_points)
        elif action == "3":
            self.freePoint(nick_name)
        elif action == "4":
            print("\n=====Casino=====\n\nВаша статистика")
            self.show_data(nick_name)
            print("Повенення до головного меню\n=====Casino=====")
            self.menu(nick_name)
        elif action == "5":
            self.data_reset(nick_name)
            print("\n=====Casino=====\nВаші дані успшішно скинуті!")
    ...
    def check_nickname(self,nick_name):
        with open("users.json") as f:
            data_name = json.load(f)
        if nick_name in data_name.keys():
            player_data = data_name[nick_name]
        else:
            player_data = self.save_new_pleyer(nick_name)
        return player_data

    def save_data(self,player_data, nick_name, game, my_points):
    ...
    def save_new_pleyer(self,nick_name):
    ...
    def Magic(self,player_data_for_magic, nick_name, my_points):
            game = "Magic"
            value, value1, value2, value3, value4, value5 = self.lvl_magic(my_points)
            ...
            while True:
                ...
                while True:
                    ...
                    if number > random_number: print("Магічне число менше", number)
                        ...
                        points = 0
                        points_1 = self.point_for_magic(counter, points, value, value1, value2, value3, value4, value5)
                        my_points += points_1
                ...
                self.save_data(player_data_for_magic, nick_name, game, my_points)
    ...
    def point_for_magic(self,counter, points, value, value1, value2, value3, value4, value5):
        if counter == 1:
    ...
    def Blackjack(self,player_data_for_Blackjack, nick_name, my_points):
        cof = lvl_blackjack()
        while True:
            while True:
                rate = input("Зробіть свою ставку aбо All in:")
                if rate != "All in"  or my_points < 0:
                    try:rate = int(rate)
                    except ValueError:continue  
                    if rate > my_points:
                        print("\nУ вас не достатьньо коштів, ви можете заробити у грі в 'Free point'")
                        continue
                    my_rate_points = rate
                    break
                my_rate_points = my_points
                break
            game = "Blackjack"
